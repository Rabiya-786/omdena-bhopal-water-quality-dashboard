...
image = Image.open(r'img\logo.png')
st.image(image)
...
data_df, df1 = load_data("data\merged_data.csv")
...
def get_Salanity(start_date, end_date):
    ...
    st.video(r"img\datcollection_dashboard.mp4", format="video/mp4")
#         st.text(
...
if selected =='Insights':
        ...
        image = Image.open(r'img\Parameter-Thresholds.png')
        st.image(image)
        st.header("Findings")
...
if selected == "Interactive Data Analysis":
    ...
    if chart_select == 'Hathaikheda dam':
        df = pd.read_csv('data\Hathaikheda.csv')
        ...
        df = pd.read_csv('data\SarangpaniLakefinal.csv')
        ...
        df = pd.read_csv(r'data\UPlake.csv')
        ...
        df = pd.read_csv('data\merged_data_lowerlake.csv')
